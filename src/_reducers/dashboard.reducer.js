...
function triggerDrawer(state = initState0, action) {
  switch (action.type) {
    case DashboardConstants.TRIGGER_DRAWER:
      return {
  ...
  switch (action.type) {
    ...
    case trackersConstants.EDIT_TRACKER:
      ...
      trackers.map((item, index) => {
        if (item.id != editableTracker.id) {
          new_trackers.push(item);
        } else {
          new_trackers.push(editableTracker);
        }
      });
      return {
        ...state,
